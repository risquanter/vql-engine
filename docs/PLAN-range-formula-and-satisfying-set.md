...
- Comment-style cleanup in files Phase 3 brought into scope (deferred boyscout,
  ...
  ("see TODOS T-002"), `QueryBinderSpec.scala` ("was a `LiteralValue`
  stopgap … PLAN Phase 5a" plus the "Phase 3/5a:" test names), and
  `TypeCheckError.scala` `UnparseableConstant` scaladoc ("Introduced by
  ADR-015 §4 / PLAN-symmetric-value-boundaries Phase 3 (closes T-002)" →
  drop the provenance sentence, keep the distinction-from-`UnknownConstantOrLiteral`
  text).
