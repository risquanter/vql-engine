 ...
 *
 * Tests the HDR (Hubbard Decision Research) PRNG integration with the Fisher-Yates sampler and
 * 4-layer seed hierarchy.
 */
class HDRSamplerSpec extends FunSuite:
  ...
  test("HDR integration with ProportionEstimator") {
    ...
    assert(
      estimate.proportion >= 0.4 && estimate.proportion <= 0.6,
