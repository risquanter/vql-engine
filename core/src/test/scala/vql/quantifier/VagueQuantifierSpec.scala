...
import vql.sampling.{HDRConfig, ProportionEstimator, SamplingParams}
...
class VagueQuantifierSpec extends FunSuite:
   ...
   *
   * After D9 (concrete types, no `[A]`), evaluation on generic populations is ProportionEstimator's
   * responsibility; VagueQuantifier only judges proportions. This helper keeps test assertions
   * readable.
   */
  ...
  test("evaluate with sampling on population") {
    ...
    assert(
      result.proportion >= 0.35 && result.proportion <= 0.65,
