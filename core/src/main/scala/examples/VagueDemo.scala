 ...
 *
 * Exercises the full public API in one pass: declare a [[TypeCatalog]], provide a [[RuntimeModel]]
 * (domain plus dispatcher), combine them into a validated [[FolModel]], then run a plain query, a
 * compound-range query with negation, and a bare-formula `satisfyingSet`. Every step returns
 * `Either[QueryError, _]`.
 */
...
private def runQuery(folModel: FolModel, label: String, queryStr: String): Unit =
  ...
  outcome match
    case Right(output) =>
      ...
      println(
        s"  proportion:  $pct (${output.result.satisfyingCount}/${output.rangeElements.size})"
      )
      println(
        s"  range:       ${output.rangeElements.map(_.raw).toList.sortBy(_.toString).mkString("{", ", ", "}")}"
      )
      println(
        s"  satisfying:  ${output.satisfyingElements.map(_.raw).toList.sortBy(_.toString).mkString("{", ", ", "}")}"
      )
    case Left(err)     =>
