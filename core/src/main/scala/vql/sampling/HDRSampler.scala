 ...
 *
 * The sole [[Sampler]] implementation. Uses Fisher-Yates shuffle powered by HDR counter-based PRNG
 * — no `scala.util.Random` anywhere.
 *
 * Based on Hubbard (2019). "A Multi-Dimensional, Counter-Based Pseudo Random Number Generator as a
 * Standard for Monte Carlo Simulations" Proceedings of the 2019 Winter Simulation Conference.
 *
 * Uses the pure-Scala hdr-rng library (cross-compiled JVM+JS), matching the Excel reference
 ...
 *
 * @param config
 *   HDR PRNG configuration (4-layer seed hierarchy per ADR-003)
 * @param ct
 *   ClassTag for creating Array[A] in Fisher-Yates shuffle
 */
class HDRSampler[A](config: HDRConfig)(using ct: ClassTag[A]) extends Sampler[A]:
   ...
   *
   * Algorithm: partial Fisher-Yates — only shuffle first n positions. For each position i in [0,
   * n), swap with random position j in [i, N), where j is determined by HDR trial(i).
   *
   * Time complexity: O(min(n, |population|)) Random values consumed: exactly n
   *
   * @param population
   *   Set of elements to sample from
   * @param n
   *   Number of elements to sample
   * @return
   *   Sampled elements (size = min(n, |population|))
   */
  def sample(population: Set[A], n: Int): Set[A] =
    ...
    else if n >= population.size then population
   ...
   *
   * HDR is deterministic: same config always produces the same sequence. To get different results,
   * change entityId, varId, seed3, or seed4 in the [[HDRConfig]].
   */
