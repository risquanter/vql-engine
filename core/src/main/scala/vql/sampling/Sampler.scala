 ...
 *
 * The sole implementation is [[HDRSampler]], which uses Fisher-Yates shuffle powered by HDR
 * counter-based PRNG. No `scala.util.Random` is used anywhere — all randomness comes from HDR per
 * ADR-003.
 *
 * @tparam A
 *   Type of elements being sampled
 */
trait Sampler[A]:
  /**
   ...
   * @return
   *   Sampled elements (size may be less than n if population is small)
   */
   ...
   *   Number of elements to sample
   * @param predicate
   *   Function to test each sampled element
   * @return
   *   (sampled elements, number of successes)
   */
...
object Sampler:
   ...
   *
   * Uses Fisher-Yates shuffle with HDR PRNG for reproducibility. This is the primary (and only)
   * sampler factory.
   *
   * @param config
