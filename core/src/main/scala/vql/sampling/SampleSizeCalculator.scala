 ...
 *
 * For sampling without replacement from finite population: n = n₀ / (1 + n₀/N)
 *
 * where n₀ is the initial sample size from infinite population formula: n₀ = (z²/4ε²)
 *
...
object SampleSizeCalculator:
   ...
   *   Size of the finite population (N)
   * @param params
   *   Sampling parameters (ε, α)
   * @return
   *   Required sample size (n), clamped to [1, N]
   */
  def calculateSampleSize(
    ...
    if populationSize == 1 then
      ...
      1
    else calculateSampleSizeInternal(populationSize, params)
  ...
  end calculateSampleSizeInternal

  /**
   * Calculate confidence interval for a proportion estimate.
   *
   * Uses Wilson score interval, which is more accurate than normal approximation for extreme
   * proportions and small samples.
   *
   * @param proportion
   *   Observed proportion (p̂) in [0, 1]
   * @param sampleSize
   *   Number of samples (n)
   * @param params
   *   Sampling parameters (α for z-score)
   * @return
  ...
  def confidenceInterval(
    ...
    else wilsonScoreInterval(proportion, sampleSize, params)
   ...
   *   Observed proportion
   * @param params
   *   Sampling parameters (ε threshold)
   * @return
   *   true if margin of error ≤ ε
   */
  def isSufficient(
    sampleSize: Int,
    proportion: Double,
   ...
   *   Target/expected proportion
   * @param actualProportion
   ...
   *
   * Pure-Scala Abramowitz & Stegun rational approximation. Maximum absolute error < 7.5 × 10⁻⁸.
   */
  private def standardNormalCDF(x: Double): Double =
    NormalApprox.cdf(x)

end SampleSizeCalculator
