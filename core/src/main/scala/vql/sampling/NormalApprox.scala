 ...
 *
 * Provides inverse CDF (quantile function) and CDF without any JVM-only dependencies, enabling
 * cross-compilation to JS.
 *
 * Both algorithms are well-known, peer-reviewed, and widely used in statistical computing
 * libraries.
 */
object NormalApprox:
   ...
   *
   * Acklam rational approximation. Maximum relative error < 1.15 × 10⁻⁹ for p ∈ (0, 1).
   *
   ...
   *
   * Abramowitz & Stegun rational approximation (formula 26.2.17). Maximum absolute error < 7.5 ×
   * 10⁻⁸.
   *
   * Reference: Abramowitz, M. and Stegun, I.A. (1964). Handbook of Mathematical Functions. National
   * Bureau of Standards.
   *
   * @param x
   *   value
