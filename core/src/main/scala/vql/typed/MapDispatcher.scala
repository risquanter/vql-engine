 ...
 * ==Motivation==
 *
 * The base [[RuntimeDispatcher]] trait requires the implementor to declare `functionSymbols` and
 * `predicateSymbols` as separate `Set[SymbolName]` values alongside the `eval*` match arms. These
 * two artefacts describe the same information from different angles and must be kept in sync by
 * convention; `validateAgainst` exists in part to catch divergences.
 *
 * `MapDispatcher` eliminates the intra-dispatcher gap: the symbol sets are derived automatically
 * from the Map keys, so adding or removing a symbol always modifies exactly one place.
 *
 * `validateAgainst` retains full value: it still enforces the catalog-vs-dispatcher cross-boundary
 * agreement (every symbol declared in the [[TypeCatalog]] has an implementation here). What changes
 * is that `validateAgainst` can no longer catch a mismatch *between* a dispatcher's own symbol-set
 * declaration and its own match arms — because that mismatch can no longer exist.
 *
 * ==When to use==
 *
 * Use `MapDispatcher` when all dispatcher logic can be expressed as independent per-symbol lambdas.
 * For dispatchers that require shared mutable state or complex cross-symbol coordination, implement
