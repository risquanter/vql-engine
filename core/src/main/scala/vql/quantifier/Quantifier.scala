 ...
 *
 * Corresponds to paper's notation: Q[~#]^{k/n} - "about k/n" Q[≥]^{k/n} - "at least about k/n"
 * Q[≤]^{k/n} - "at most about k/n"
 *
 * OCaml-style ADT pattern: enum for sum type (like Term, Formula from Harrison's formulas.ml)
 *
 * OCaml reference: formulas.ml type ('a)formula = False | True | Atom of 'a | ...
 *
 * Paper reference: Definition 1 (Section 5.2) "We consider vague quantifiers of the form
 * Q[op]^{k/n} where:
...
object Quantifier:
   ...
   *
   * OCaml reference: formulas.ml let mk_and p q = And(p,q) and mk_or p q = Or(p,q)
   *
   ...
   *
   * Paper reference: Example 3 (Section 5.2) "We use the following quantifiers in our examples..."
   */
   ...
   *
   * OCaml pattern: accessor function OCaml reference: fol.ml has helper functions for data
   * extraction
   */
   ...
   *
   * OCaml pattern: predicate function with pattern matching OCaml reference: fol.ml let rec holds
   * (domain,func,pred as m) v fm = match fm with ...
   *
   * Paper reference: Definition 2 (Section 5.2) "A vague query Q x (R, φ) is satisfied with
   * precision ε when:
   *   - Q[~#]^{k/n}: |Prop_D - k/n| ≤ ε
   *   - Q[≥]^{k/n}: Prop_D ≥ k/n - ε
   *   - Q[≤]^{k/n}: Prop_D ≤ k/n + ε"
   ...
   *   Quantifier type
   * @param prop
   *   Observed proportion from sample
   * @param epsilon
   *   Precision parameter from sampling (Section 4)
   * @return
   *   true if quantifier accepts proportion
   */
   ...
   *
   * Returns the proportion interval within which `accepts` returns true. Clamped to [0, 1].
   *
   ...
   *
   * OCaml pattern: printer function OCaml reference: fol.ml has print_fol_formula
   */
