 ...
 * Semantics" Section 5.2: Vague Quantifiers and Their Semantics
 *
 * Implements Definition 2 (Sampled Answer Semantics): A quantifier Q evaluates to true if the
 * estimated proportion of elements satisfying the predicate falls within Q's acceptance range.
 *
 ...
 *
 * This is the ergonomic Scala API (percentage-based). Internally, acceptance checking delegates to
 * [[vql.quantifier.Quantifier.accepts]] which implements the paper's Definition 2 with ratio-based
 * notation. Use [[toQuantifier]] to convert to the canonical ratio form.
 */
sealed trait VagueQuantifier:
   ...
   *
   * The ratio form (k/n) is what the parser produces and what the paper uses. This conversion
   * enables the typed DSL and the string-parsed path to share a single acceptance-checking
   * implementation.
   */
   ...
   *
   * Delegates to [[vql.quantifier.Quantifier.accepts]] using the quantifier's own tolerance as
   * epsilon.
   *
   * @param proportion
   *   Estimated proportion in [0, 1]
   * @return
   *   true if proportion falls within quantifier's acceptance range
   */
 ...
 *
 * Accepts proportions close to a target value within tolerance. Example: "about half" =
 * Approximately(0.5, 0.1) accepts [0.4, 0.6]
 *
 * @param target
 *   Target proportion in [0, 1]
 * @param tolerance
 *   Acceptable deviation from target (default: 0.1 = ±10%)
 */
 ...
 *
 * Accepts proportions greater than or equal to threshold. Example: "most" = AtLeast(0.7) accepts
 * [0.7, 1.0]
 *
 * @param threshold
 *   Minimum acceptable proportion in [0, 1]
 */
 ...
 *
 * Accepts proportions less than or equal to threshold. Example: "few" = AtMost(0.3) accepts [0,
 * 0.3]
 *
 * @param threshold
 *   Maximum acceptable proportion in [0, 1]
 */
...
object VagueQuantifier:
  ...
  /**
   * Convert from the canonical ratio-based [[vql.quantifier.Quantifier]] to the ergonomic
   * percentage-based [[VagueQuantifier]].
   *
   * This is the inverse of [[VagueQuantifier.toQuantifier]]. Used by the bridge layer when
   * string-parsed queries need to flow through the typed-DSL evaluation pipeline.
   */
