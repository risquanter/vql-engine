 ...
 *
 * @param satisfied
 *   Whether the quantifier accepts the observed proportion
 * @param proportion
 *   Observed or estimated proportion in [0, 1]
 * @param confidenceInterval
 *   Wilson score CI (exact mode: degenerate [p, p])
 * @param quantifier
 *   The vague quantifier that was evaluated
 * @param domainSize
 *   Total population/domain size
 * @param sampleSize
 *   Number of elements actually examined (= domainSize for exact)
 * @param satisfyingCount
 *   Number of elements satisfying the predicate
 * @param estimate
 *   Full ProportionEstimate with margin of error and params
 */
case class VagueQueryResult(
   ...
   *
   * Result is significant when the entire confidence interval falls within (if satisfied) or
   * outside (if not satisfied) the quantifier's acceptance range.
   *
   * For exact evaluation (sampleSize == domainSize), the CI is degenerate so this always returns
   * true.
   */
...
object VagueQueryResult:
   ...
   *
   * @param quantifier
   *   The vague quantifier to evaluate against
   * @param estimate
   *   Proportion estimate (carries successes, CI, etc.)
   * @param domainSize
   *   Total population size (may differ from sampleSize)
   */
  def fromEstimate(
    quantifier: VagueQuantifier,
    estimate: ProportionEstimate,
