...
/**
 * Structural membership test over a parsed `Formula[FOL]` — the inert tree `FOLParser.parse`
 * returns. It forks no parser (ADR-007), needs no `TypeCatalog`, and embeds no sort logic. Callers
 * with query text parse via `FOLParser.parse` and map the foundation `ParseError` at their own
 * boundary, the same input discipline as `VagueSemantics.satisfyingSet` (ADR-017 §6).
 */
object FragmentCheck:

  /**
   * `Right(())` iff `formula` lies in `fragment`; `Left` carries the first violated rule in
   * pre-order, left-to-right traversal (ADR-018).
   */
  ...
  /**
   * No quantifier node anywhere, and no `Term.Fn` in any atom's terms. The quantifier node is
   * reported before its body (pre-order), and within an atom the leftmost offending term wins.
   */
  ...
  /**
   * The first `Term.Fn` in `terms`, left-to-right. An outermost `Fn` is a violation on its own, so
   * its arguments are not inspected. `Var` and `Const` are admitted.
   */
  ...
  /**
   * Maximum number of `Forall`/`Exists` nodes on any single root-to-leaf path. Nesting adds one
   * level; siblings take the larger. Atom terms carry no quantifiers, so they contribute 0.
   */
