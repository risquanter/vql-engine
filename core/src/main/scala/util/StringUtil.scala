 ...
 *
 * OCaml uses character lists for string manipulation. These functions bridge between Scala's String
 * and List[Char].
 */
object StringUtil:
   ...
   *
   * OCaml implementation: let explode s = let rec exap n l = if n < 0 then l else exap (n - 1)
   * (s.[n] :: l) in exap (String.length s - 1) []
   *
   * Example: explode("abc") == List('a', 'b', 'c')
   */
   ...
   *
   * OCaml implementation: let implode l = itlist (^) l ""
   *
   * Example: implode(List('a', 'b', 'c')) == "abc"
   */
   ...
   *
   * OCaml implementation: let matches s = let chars = explode s in fun c -> mem c chars
   *
   * Returns a function that checks if a character is in the given string.
   *
   * Example: val isVowel = matches("aeiou") isVowel('a') == true isVowel('b') == false
   */
   ...
   *
   * TODO: OVERLAP ISSUE - The characters . , ; appear in BOTH symbolic and punctuation! This causes
   * them to be treated as symbolic (checked first in lexer), allowing multi-char tokens like "..."
   * or ",,," instead of individual tokens.
   *
   * Consider removing . , ; from symbolic to make them single-char only: val symbolic =
   * matches("~`!@#$%^&*-+=|\\:<>?/")
   *
   * This would ensure:
   ...
   *
   * OCaml uses: forall numeric (explode s) Scala: forall(numeric, explode(s))
   */
  def forall(pred: Char => Boolean, chars: List[Char]): Boolean =
    chars.forall(pred)

  /**
   * Check if string consists only of numeric characters Used in fol.ml: is_const_name
   */
   ...
   *
   * Shared by [[parser.TermParser]] and [[vql.parser.VagueQueryParser]] to avoid regex duplication.
   * The single-backslash form `\d` is required — raw strings with `\\d` do not match the regex `\d`
   * character class.
   */
